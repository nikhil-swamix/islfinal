...
class Slide extends React.Component {
    ...
    async getData() {
        const response = await axios.get('https://islec.herokuapp.com/api/carousels/eee');
        try {
            this.setState({
                ...
                <Carousel>
                    {!isLoading ? (
                        carousel_list.map((carousel) => {
                            ...
                            return (
                                <Carousel.Item>
                                    <img key={_id} src={img} className="img-fluid d-block w-100" alt="CE Carousel Images" />
                                    {!descp ? (
                                        ' '
                                    ) : (
                                        <Carousel.Caption>
