...
class Syllabus extends React.Component {
    ...
    async getData() {
        const response = await axios.get('https://islec.herokuapp.com/api/syllabs/cse');
        try {
            this.setState({
            ...
            <>
                {!isLoading ? (
                    <div className="table-responsive mt-10">
                        <DataTable keys="id" columns={tableColumns} initialData={syllabus_list} initialPageLength={8} pageLengthOptions={[5, 20, 30, 40, 50, 60]} />
                    </div>
                ) : (
                    <>
